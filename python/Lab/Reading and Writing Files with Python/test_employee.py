...
assert len(employees) == 2, f"1.Expected 2 employees, but got {len(employees)}"

assert (
    Employee.get_at_line(1, file_name).__dict__ == employees[0].__dict__
), f"2.Expected Employee.get_at_line(1) to return {employees[0].__dict__} but got {Employee.get_at_line(1, file_name).__dict__}"
...
), f"3.Expected 3 employees, but got {len(Employee.get_all(file_name))}"
...
), f"4.Expected phone number to be 555-555-5555, but got {new_president.phone_number}"
