...
), f"1.Expected 'Bruce Wayne - CEO\nbwayne@example.com' but got {manager_1.email_signature()}"

assert (
    manager_1.email_signature(include_phone=True)
    == "Bruce Wayne - CEO\nbwayne@example.com"
), f"2.Expected 'Bruce Wayne - CEO\nbwayne@example.com' but got {manager_1.email_signature(include_phone=True)}"

assert (
    manager_1.meetings == []
), f"3.Expected meetings to be [], but got {manager_1.meetings}"
...
), f"4.Expected first meeting to be '{expected_meeting}' but got {manager_1.meetings[0]}"

result = manager_1.run_next_meeting()
assert (
    result == expected_meeting
), f"5.Expected run_next_meeting to return '{expected_meeting}' but got {result}"

assert (
    len(manager_1.meetings) == 2
), f"6.Expected number of meetings to be '2', but was {manager_1.meetings}"
